...
//  }
  ...
  public Rect getFramingRect() {
    ...
    if (framingRect == null) {
      ...
      TypedArray a = context.obtainStyledAttributes(null, R.styleable.ScannerCamera,0,defFramingStyleId);
      int width = a.getInt(R.styleable.ScannerCamera_sc_width, 180);
      int height = a.getInt(R.styleable.ScannerCamera_sc_height, 180);
      int resbase = a.getInt(R.styleable.ScannerCamera_sc_baseres, 640);
      int left = a.getInt(R.styleable.ScannerCamera_sc_left, 0);
      int top = a.getInt(R.styleable.ScannerCamera_sc_top, 0);
      boolean mIsFullScreen = a.getBoolean(R.styleable.ScannerCamera_sc_fullscreen, false);
      ...
      a.recycle();
      int newWidth = width * screenResolution.x / resbase;
      left = left * newWidth / width;
      if (left <= 0) {//代表居中
        left = (screenResolution.x - newWidth) / 2;
      }
      int newHeight = height * newWidth / width;
      top = top * newWidth / width;
      if (top <= 0) {//代表居中
        int mStatusBarHeight = getStatusBarHeight(context);
        if (mIsFullScreen) {
          mStatusBarHeight = 0;
        }
        top = (screenResolution.y - newHeight) / 2 - mStatusBarHeight
                / 2;
      }
